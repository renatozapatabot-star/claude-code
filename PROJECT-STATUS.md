...
| WO-19 | 🚨🚨 **HIGHEST PRIORITY** — real, live security defects in `evco-portal`, independently verified by direct code read: 7 real employee passwords hardcoded in plaintext (`firstname+2026` pattern); `/api/*` routes bypass session auth entirely at the middleware layer; a live-shaped Supabase JWT hardcoded as a fallback in a committed script; **and the real evco-portal repo has 176 branches this canon's WO-16 diagnosis never checked — the most recent relevant one does NOT fix WO-16's root cause, it only renames the hardcoded list.** | **OPEN — rotate the 7 passwords out of band immediately, independent of any code fix.** See §5.4's dedicated entry for the full diagnosis. |

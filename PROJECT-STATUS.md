...
| WO-16 | 🚨 SEV-1 MAFESA/EVCO cross-tenant fix, write access | **OPEN — APPLY NOW (reversed 2026-07-31).** Fix is a verified, COMPLETE, ready-to-apply patch (`evidence/evco-cross-tenant-fix.patch`, covers all 8 real call sites, `tsc`-clean); `evco-portal` untouched, awaiting founder write-access confirmation. A related, separate tenant-scoping gap in `opportunities.ts` was also found and fixed (`evidence/evco-opportunities-tenant-fix.patch`). A deeper recon confirmed the bug is still live on `origin/main` today; three unmerged real fix attempts exist but none has an open PR, and `main` hasn't merged anything in 28 days — there is no active coordination to wait for, so "reconcile before applying" was reversed to "apply as an emergency stopgap now." See WO-19/WO-20. |
...
| WO-19 | 🚨🚨 **HIGHEST PRIORITY** — real, live security defects in `evco-portal`, independently verified by direct code read: 7 real employee passwords hardcoded in plaintext (`firstname+2026` pattern); `/api/*` routes bypass session auth entirely at the middleware layer; a live-shaped Supabase JWT hardcoded as a fallback in a committed script; **and the real evco-portal repo has 176 branches — three separate unmerged fix attempts for WO-16's bug exist, but none has an open PR against `main`.** | **OPEN — rotate the 7 passwords out of band immediately, independent of any code fix; separately, decide which tenant-isolation fix ships to `main` (this session's WO-16 patch, or one of two real fixes already unmerged in the repo) — ship SOMETHING now.** See §5.4's dedicated entry for the full diagnosis. |
| WO-20 | 🚨 Governance/process finding — not a code bug: all 177 branches/85 PRs in `evco-portal` are authored under one identity, with strong evidence of multiple AI agent sessions and essentially no human review gate, and `main` has not merged anything in 28 days despite ready PRs sitting open. This is the real root cause behind why WO-16/19's fixes exist but never ship. | **OPEN — founder must choose: (a) a real human-gated merge cadence, or (b) designate one AI session/tool as sole merge authority.** Acceptance: a real merge lands on `main` within 7 days. See §5.4. |
